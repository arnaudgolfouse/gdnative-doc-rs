...
pub trait Callbacks {
    ...
    /// Called before encoding each file. No [`Event`] has been processed yet, so
    /// the `String` should be empty.
    ///
    /// **Default**: does nothing
    fn start_file(&mut self, _s: &mut String) {}
...
impl<'a> Generator<'a> {
    ...
    pub(crate) fn generate_root_file(&mut self, extension: &str) -> String {
        ...
        self.callbacks.start_file(&mut root_file);
    ...
    pub(crate) fn generate_files(&mut self) -> Vec<(String, String)> {
        ...
        for (name, class) in &self.documentation.classes {
            ...
            callbacks.start_file(&mut class_file);
